...
it('keeps the singing item compact until editing begins', () => {
  const onSave = vi.fn();
  render(<ThemeProvider theme={theme}><SingingListPanel entries={[entry]} filter="ALL" onFilterChange={() => undefined} onSave={onSave} onRemove={() => undefined} /></ThemeProvider>);
...
it('uses compact status chips to filter the list', () => {
  ...
  render(<ThemeProvider theme={theme}><SingingListPanel entries={[entry]} filter="ALL" onFilterChange={onFilterChange} onSave={() => undefined} onRemove={() => undefined} /></ThemeProvider>);
