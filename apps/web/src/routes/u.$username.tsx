      ...
      {profileQuery.isLoading ? <Stack alignItems="center" py={10}><CircularProgress aria-label="Loading public profile" /></Stack>
        : profileQuery.isError || !profile ? <Alert severity="info"><Typography fontWeight={800}>Public profile not found</Typography>This user has no public lists, or the address is incorrect.</Alert>
          : <Stack spacing={3}>
            ...
            {profile.lists.singingList === 'PUBLIC' && <Paper component="section" sx={{ p: 3 }} aria-labelledby="public-singing-heading"><Stack direction="row" justifyContent="space-between" alignItems="center"><Box><Typography id="public-singing-heading" variant="h2" fontSize="1.55rem">Practice Library</Typography><Typography variant="body2" color="text.secondary">Personal notes stay private.</Typography></Box><Chip label="Public" color="success" size="small" /></Stack>{singingQuery.isLoading ? <CircularProgress size={20} sx={{ mt: 3 }} /> : <List disablePadding sx={{ mt: 1 }}>{(singingQuery.data ?? []).map((entry) => <ListItem key={entry.id} divider disableGutters><ListItemText primary={entry.title} secondary={entry.artist} primaryTypographyProps={{ fontWeight: 700 }} /><Chip label={statusLabels[entry.status]} size="small" variant="outlined" /></ListItem>)}</List>}</Paper>}
          </Stack>}
    </Container></Box>
  </>;
}
